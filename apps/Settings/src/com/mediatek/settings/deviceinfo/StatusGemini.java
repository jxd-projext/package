...
public class StatusGemini extends PreferenceActivity {
    ...
    // Elink_liaobz add 2013-12-4
    private static final String KEY_HARDWARE_INFO = "hw_info";
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (!sIsWifiOnly) {
            ...
            // Elink_liaobz add 2013-12-3
            String serial = mGeminiPhone.getSN();// Build.SERIAL;
        ...
        try {
            ...
            if (!file.exists()) {
                file = new File("/sys/devices/system/cpu/present");
            }
            if (file.exists()) {
                BufferedReader  reader=new BufferedReader(new FileReader(file));
                hw_info = reader.readLine();
            }
        } catch (Exception e) {}
        Xlog.e(TAG, "liaobz:hw_info = " + hw_info);
        if ("0-1".equals(hw_info)) {
            setSummaryText(KEY_HARDWARE_INFO, "MT8312");
        } else if ("0-3".equals(hw_info)) {
            setSummaryText(KEY_HARDWARE_INFO, "MT8382");
        } else {
            setSummaryText(KEY_HARDWARE_INFO, "MT83X2");
        }
		// Elink_liaobz add 2013-12-4 >>
    }
