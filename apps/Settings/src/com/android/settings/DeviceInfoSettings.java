    ...
    public void onCreate(Bundle icicle) {
        ...
        if (isApkExist(act, "com.adups.fota") == false){
            if(findPreference("adupsfota_software_update") != null) {
                parentPreference.removePreference(findPreference("adupsfota_software_update"));
            }
        }
        if (isApkExist(act, "com.redbend.dmClient") == false) {
            if(findPreference("gmt_software_update") != null) {
                parentPreference.removePreference(findPreference("gmt_software_update"));
            }
        }
        if (isApkExist(act, "com.adups.fota") || isApkExist(act, "com.redbend.dmClient")) {
            ...
            if(findPreference("mota_system_update") != null){
                parentPreference.removePreference(findPreference("mota_system_update"));
            }
            if(findPreference("scomo") != null){
                parentPreference.removePreference(findPreference("scomo"));
            }
            if(findPreference("additional_system_update_settings") != null){
                parentPreference.removePreference(findPreference("additional_system_update_settings"));
            }
            if(findPreference("more_software_updates") != null){
                parentPreference.removePreference(findPreference("more_software_updates"));
            }
        }
        //adupsfota end
    }
    ...
    */
    ...
    private boolean isApkExist(Context ctx, String packageName){
        ...
        try {
            ...
            Log.e("AdupsFota", "isApkExist not found");
            return false;
        }

        if (versionName != null) {
            String[] names = versionName.split("\\.");
            if (names.length >= 3 && "9".equals(names[2])) {
                return false;
            }
        }
        Log.i("AdupsFota", "isApkExist = true");
        return true;
    }
    //adupsfota end
}
